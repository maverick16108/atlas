...
<template>
  <div class="min-h-screen bg-dark-900 text-white selection:bg-gold-500 selection:text-white relative" :class="[showAccreditation ? 'overflow-x-hidden' : 'h-screen overflow-hidden']">
    ...
    <!-- Navbar -->
    <nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 backdrop-blur-xl bg-dark-900/60 border-b border-gold-500/20 shadow-[0_4px_20px_rgba(212,175,55,0.15)]">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
          <!-- Logo -->
          <div class="flex-shrink-0 flex items-center cursor-pointer space-x-3 group relative" @click="router.push('/')">
            ...
            <div class="hidden sm:flex flex-col items-center overflow-hidden">
               ...
               <div class="font-oswald font-light text-sm leading-none tracking-[0.45em] text-gold-400 mt-1 uppercase ml-[2px] flex">
                  <span 
          ...
          <div class="flex items-center space-x-4">
            <button @click="toggleLanguage" class="text-gray-400 hover:text-white font-medium transition-colors">
              {{ locale === 'ru' ? 'EN' : 'RU' }}
            </button>
      ...
      <div class="max-w-7xl mx-auto px-4 relative z-10">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-8 mb-16 items-start">
          ...
          <div class="flex justify-center w-full">
             <div v-if="showAccreditation" class="w-full max-w-[260px] relative group">
                <!-- Aura -->
                ...
                <div class="relative bg-gradient-to-b from-[#FFF5C3] via-[#D4AF37] to-[#B8860B] rounded-xl p-[1px] shadow-[0_10px_30px_-5px_rgba(212,175,55,0.4)] transform transition-transform duration-300 hover:-translate-y-2 hover:shadow-[0_25px_50px_-10px_rgba(212,175,55,0.5)]">
                    ...
                    <div class="relative bg-gradient-to-br from-[#F5D061] via-[#E1B32A] to-[#C59B25] rounded-[11px] p-5 flex flex-col items-center text-center gap-3 overflow-hidden">
                         ...
                         <!-- Button -->
                         <button @click="openModal" class="relative z-20 w-full bg-dark-900 text-[#F5D061] hover:text-white hover:bg-black px-4 py-2.5 rounded-lg font-bold uppercase tracking-wider text-[10px] transition-all shadow-md hover:shadow-lg border border-white/5 mt-1">
                             {{ t('form.submit') }}
                         </button>
                    </div>
                </div>
          ...
          <div class="lg:text-right space-y-4">
            <h4 class="text-lg font-kanit font-bold text-gold-400 uppercase tracking-widest mb-6">{{ t('footer.contacts') }}</h4>
    ...
    <BaseModal :is-open="isModalOpen" :title="t('form.title')" @close="closeModal">
        <div class="space-y-4">
            <p class="text-gray-400 text-sm mb-4">{{ t('form.subtitle') }}</p>
            <BaseInput ref="orgInput" :label="t('form.org_label')" :placeholder="t('form.org_placeholder')" premium />
            <BaseInput :label="t('form.phone_label')" placeholder="+7 (999) 000-00-00" premium />
            <BaseInput :label="t('form.email_label')" placeholder="corp@atlas.gold" v-model="email" premium />
            <BaseButton block variant="primary" class="mt-6" @click="closeModal">{{ t('form.submit') }}</BaseButton>
        </div>
    </BaseModal>
